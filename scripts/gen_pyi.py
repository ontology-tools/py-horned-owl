...
def handle_module(module: str, py_imports: list[str], pyi_imports: list[str]):
    m = getattr(pho, module) if module else pho
    with open(f"pyhornedowl/{module}/__init__.py", "w") as f:
        members = [k for k, v in m.__dict__.items() if isinstance(v, type) or callable(v)]
        ...
        if module:
            ...
            f.write("from .pyhornedowl import " + ", ".join(members) + "\n")


        f.write("\n\n")
            
        f.write("__all__ = [")
        f.write(", ".join([f'"{n}"' for n in members]))
        f.write("]\n")

    with open(f"pyhornedowl/{module}/__init__.pyi", "w") as f:
        f.writelines([f"{line}\n" for line in pyi_imports])
        f.write("\n")
            ...
            if isinstance(entry, type):
                ...
                for member_name, member in entry.__dict__.items():
                    ...
                    if isinstance(member, entry):
                        ...
                        if (
                            member_name in member_docs
                            or hasattr(member, "__doc__")
                            and member.__doc__ is not None
                        ):
                            doc = member_docs.get(
                                member_name, getattr(member, "__doc__")
                            )
                            f.write('    """\n')
                    ...
                    if hasattr(member, "__doc__"):
                        ...
                        if doc is not None:
                            ...
                            if len(lines) > 2:
                                annotations_end = lines.index(
                                    next(x for x in lines if not x.startswith("@")), 0
                                )
                                ...
                                if callable(member):
                                    ...
                                    doc = "\n".join(
                                        [
                                            f"        {l}"
                                            for l in lines[annotations_end + 2 :]
                                        ]
                                    )
                                    f.write(
                                        f'        """\n{doc}\n        """\n        ...\n\n'
                    ...
                    if callable(member):
                        f.write(
                            f"    def {member_name}{inspect.signature(member)}:\n        ...\n\n"
                        )
                ...
                if hasattr(entry, "__doc__"):
                    ...
                    if doc is not None:
                        ...
                        if len(lines) > 2:
                            ...
                            f.write(f"def {sign}:\n")
                            doc = "\n".join(
                                [f"    {l}" for l in lines[annotations_end + 2 :]]
                            )
                            f.write(f'    """\n{doc}\n    """\n    ...\n\n')
...
def handle_template_module(module: str):
    with open(f"pyhornedowl/{module}/__init__.py", "w") as f:
        f.write(f"from ..pyhornedowl import {module}\n")
        ...
        for name, entry in getattr(pho, module).__dict__.items():
            if (
                not isinstance(entry, type)
                and not type(entry) == typing._UnionGenericAlias
            ):
                continue
