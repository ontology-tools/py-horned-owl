...
def test_explicit_fail():
    o = pyhornedowl.open_ontology_from_file(
        r(f"simple.owl"), index_strategy=IndexCreationStrategy.Explicit
    )
...
def test_on_load():
    o = pyhornedowl.open_ontology_from_file(
        r(f"simple.owl"), index_strategy=IndexCreationStrategy.OnLoad
    )
