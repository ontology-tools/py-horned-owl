...
impl PyIndexedOntology {
    pub fn reason_subclasses_of(&self, iri: String) -> Vec<String> {
        let mut set_ontology = horned_owl::ontology::set::SetOntology::new();

        for ax in self.ontology.iter() {
            set_ontology.insert(ax.clone());
        }

        let trans = translate_ontology(&set_ontology);
        let state = assert(&trans);

        return state.named_subsumptions().iter()
            .filter_map(|(sub, sup)| if sub.id == iri { Some(sup.id.clone()) } else { None })
            .collect();
    }
