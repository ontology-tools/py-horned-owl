...
-------------------------
The type stubs are generated by PyO3 itself: with its ``experimental-inspect`` feature, the compiled module carries a description of every class, function and signature, and ``maturin build --generate-stubs`` (or ``maturin develop --generate-stubs``, as ``make dev`` does) turns it into ``.pyi`` files next to the native module ``pyhornedowl/pyhornedowl``. Rust types with a hand-written conversion declare their Python type through ``FromPyObject::INPUT_TYPE`` and ``IntoPyObject::OUTPUT_TYPE``.

PyO3 cannot express type aliases, so the unions such as ``ClassExpression`` or ``Component`` are defined in Python: ``scripts/build_model.py`` writes them to ``pyhornedowl/model/__init__.py`` from ``template/model.json``, together with the Rust model.